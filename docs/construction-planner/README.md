...
rewrite. Everything in this folder is meant to be copied into a new repo of its
own.

## Hand this over

Copy this whole folder into a fresh repo (`construction-planner`), then move
`CLAUDE.md` to the repo root:

```
construction-planner/
  CLAUDE.md            ← conventions. Claude Code reads this every session.
  index.html           ← the shell. No build step: open it and it runs.
  ...
  tools/run_tests.js   ← node tools/run_tests.js  — must exit 0 before commit.
  tools/test_cxc_model.js
```

Then copy these from cx-portal **unchanged**, so the two apps share primitives
and the merge is a file move: `icons.js`, `format.js`, `cx-state.js`,
`cx-store.js`, `cx-actions.js`, `DESIGN_TOKENS.md`, and the `:root` token block
at the top of `styles.css` (a trimmed copy is already inlined in `index.html`).

Verify it works:

```
node tools/run_tests.js       # 50 assertions, exits 0
